...
def train(args, trainloader, testloader):
    ...
    for epoch in range(start_epoch, 300):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            loss_1 = criterion(outputs, labels.cuda())
            loss_1.backward(retain_graph=True)


            gradient_input = inputs.grad.data
            gradient_input_sign = 2 * (gradient_input > 0).float() - 1.0
            inputs_adversarial = inputs + gradient_input_sign * 0.1

            outputs_adversarial = model(inputs_adversarial.cuda())
            loss_2 = criterion(outputs_adversarial, labels.cuda())
